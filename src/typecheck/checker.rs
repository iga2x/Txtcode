...
use crate::typecheck::types::{Type, TypeContext, FunctionType};
...
impl TypeChecker {
    ...
    fn check_statement(&mut self, stmt: &Statement) {
        match stmt {
            Statement::Assignment { pattern, type_annotation, value, .. } => {
                ...
                let value_type = match inference.infer_expression(value) {
                    ...
                    crate::typecheck::types::InferenceResult::Unknown => {
                        self.errors.push("Cannot infer type for assignment value".to_string());
                        return;
                    }
                };
            ...
            Statement::FunctionDef { name: _name, params, return_type, body, .. } => {
            ...
            Statement::DoWhile { body, condition, .. } => {
                ...
                match inference.infer_expression(condition) {
                    crate::typecheck::types::InferenceResult::Known(ty) => {
                        if !ty.is_compatible_with(&Type::Bool) {
                            self.errors.push("Do-while condition must be boolean".to_string());
                        }
                    }
                    crate::typecheck::types::InferenceResult::Error(msg) => {
                        self.errors.push(format!("Do-while condition error: {}", msg));
                    }
                    _ => {}
                }
            }
                ...
                match inference.infer_expression(iterable) {
                    crate::typecheck::types::InferenceResult::Known(ty) => {
                        match ty {
                            ...
                            _ => {
                                self.errors.push("For loop iterable must be array, map, set, or string".to_string());
                            }
                        }
                    }
                    crate::typecheck::types::InferenceResult::Error(msg) => {
                        self.errors.push(format!("For loop iterable error: {}", msg));
                    }
                    _ => {}
                }
