...
impl RegexLib {
    ...
    pub fn call_function(name: &str, args: &[Value]) -> Result<Value, RuntimeError> {
        match name {
            "regex_match" | "regex_test" => {
                if args.len() != 2 {
                    return Err(RuntimeError::new("regex_match requires 2 arguments (pattern, text)".to_string()));
                }
                match (&args[0], &args[1]) {
                    (Value::String(pattern), Value::String(text)) => {
                        Self::regex_match(pattern, text)
                    }
                    _ => Err(RuntimeError::new("regex_match requires string arguments".to_string())),
                }
            }
            "regex_find" | "regex_search" => {
                if args.len() != 2 {
                    return Err(RuntimeError::new("regex_find requires 2 arguments (pattern, text)".to_string()));
                }
                match (&args[0], &args[1]) {
                    (Value::String(pattern), Value::String(text)) => {
                        Self::regex_find(pattern, text)
                    }
                    _ => Err(RuntimeError::new("regex_find requires string arguments".to_string())),
                }
            }
            "regex_find_all" | "regex_findall" => {
                if args.len() != 2 {
                    return Err(RuntimeError::new("regex_find_all requires 2 arguments (pattern, text)".to_string()));
                }
                match (&args[0], &args[1]) {
                    (Value::String(pattern), Value::String(text)) => {
                        Self::regex_find_all(pattern, text)
                    }
                    _ => Err(RuntimeError::new("regex_find_all requires string arguments".to_string())),
                }
            }
            "regex_replace" => {
                if args.len() != 3 {
                    return Err(RuntimeError::new("regex_replace requires 3 arguments (pattern, text, replacement)".to_string()));
                }
                match (&args[0], &args[1], &args[2]) {
                    (Value::String(pattern), Value::String(text), Value::String(replacement)) => {
                        Self::regex_replace(pattern, text, replacement)
                    }
                    _ => Err(RuntimeError::new("regex_replace requires string arguments".to_string())),
                }
            }
            "regex_replace_all" => {
                if args.len() != 3 {
                    return Err(RuntimeError::new("regex_replace_all requires 3 arguments (pattern, text, replacement)".to_string()));
                }
                match (&args[0], &args[1], &args[2]) {
                    (Value::String(pattern), Value::String(text), Value::String(replacement)) => {
                        Self::regex_replace_all(pattern, text, replacement)
                    }
                    _ => Err(RuntimeError::new("regex_replace_all requires string arguments".to_string())),
                }
            }
            "regex_split" => {
                if args.len() != 2 {
                    return Err(RuntimeError::new("regex_split requires 2 arguments (pattern, text)".to_string()));
                }
                match (&args[0], &args[1]) {
                    (Value::String(pattern), Value::String(text)) => {
                        Self::regex_split(pattern, text)
                    }
                    _ => Err(RuntimeError::new("regex_split requires string arguments".to_string())),
                }
            }
            _ => Err(RuntimeError::new(format!("Unknown regex function: {}", name))),
        }
    }
    ...
    fn regex_find(pattern: &str, text: &str) -> Result<Value, RuntimeError> {
        ...
        if let Some(captures) = re.captures(text) {
            if let Some(matched) = captures.get(0) {
                ...
                result.insert("match".to_string(), Value::String(matched.as_str().to_string()));
    ...
    fn regex_replace_all(pattern: &str, text: &str, replacement: &str) -> Result<Value, RuntimeError> {
        let re = Regex::new(pattern)
            .map_err(|e| RuntimeError::new(format!("Invalid regex pattern: {}", e)))?;
