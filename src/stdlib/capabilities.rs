...
impl CapabilityLib {
    ...
    pub fn call_function<E: CapabilityExecutor>(
        name: &str,
        args: &[Value],
    ...
    fn grant_capability<E: CapabilityExecutor>(
        executor: &mut E,
        args: &[Value],
    ) -> Result<Value, RuntimeError> {
        if args.len() < 2 {
            return Err(RuntimeError::new(
                "grant_capability requires at least 2 arguments: resource, action".to_string(),
            ));
        }
        ...
        let resource_str = match &args[0] {
            ...
            _ => {
                return Err(RuntimeError::new(
                    "grant_capability: first argument must be a string (resource)".to_string(),
                ))
            }
        };
        ...
        let scope = if args.len() > 2 {
            match &args[2] {
                ...
                _ => {
                    return Err(RuntimeError::new(
                        "grant_capability: third argument must be a string (scope) or null"
                            .to_string(),
                    ))
                }
            }
    ...
    fn use_capability<E: CapabilityExecutor>(
        executor: &mut E,
        args: &[Value],
    ) -> Result<Value, RuntimeError> {
        if args.is_empty() {
            return Err(RuntimeError::new(
                "use_capability requires 1 argument: token_id".to_string(),
            ));
        }
        ...
        let token_id = match &args[0] {
            ...
            _ => {
                return Err(RuntimeError::new(
                    "use_capability: argument must be a string (token_id)".to_string(),
                ))
            }
        };
    ...
    fn revoke_capability<E: CapabilityExecutor>(
        executor: &mut E,
        args: &[Value],
    ) -> Result<Value, RuntimeError> {
        if args.is_empty() {
            return Err(RuntimeError::new(
                "revoke_capability requires at least 1 argument: token_id".to_string(),
            ));
        }
        ...
        let token_id = match &args[0] {
            ...
            _ => {
                return Err(RuntimeError::new(
                    "revoke_capability: first argument must be a string (token_id)".to_string(),
                ))
            }
        };
        ...
        let reason = if args.len() > 1 {
            match &args[1] {
                ...
                _ => {
                    return Err(RuntimeError::new(
                        "revoke_capability: second argument must be a string (reason) or null"
                            .to_string(),
                    ))
                }
            }
    ...
    fn capability_valid<E: CapabilityExecutor>(
        executor: &mut E,
        args: &[Value],
    ) -> Result<Value, RuntimeError> {
        if args.is_empty() {
            return Err(RuntimeError::new(
                "capability_valid requires 1 argument: token_id".to_string(),
            ));
        }
        ...
        let token_id = match &args[0] {
            ...
            _ => {
                return Err(RuntimeError::new(
                    "capability_valid: argument must be a string (token_id)".to_string(),
                ))
            }
        };
    ...
    fn parse_resource(
        resource_str: &str,
        action: &str,
    ) -> Result<PermissionResource, RuntimeError> {
        match resource_str {
            ...
            _ => Err(RuntimeError::new(format!(
                "Unknown resource type: {}. Expected 'fs', 'net', 'sys', or 'process'",
                resource_str
            ))),
        }
    }
    ...
    fn parse_duration(duration_str: &str) -> Result<Option<std::time::Duration>, RuntimeError> {
            ...
            return Ok(Some(std::time::Duration::from_secs(
                duration_str.parse().map_err(|_| {
                    RuntimeError::new(format!(
                        "Invalid duration format: {}. Expected format like '10s', '5m', '1h'",
                        duration_str
                    ))
                })?,
            )));
        ...
        let value: u64 = value_str.parse().map_err(|_| {
            RuntimeError::new(format!(
                "Invalid duration value: {}. Expected a number",
                value_str
            ))
        })?;
