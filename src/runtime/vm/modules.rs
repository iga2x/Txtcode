...
impl VirtualMachine {
    ...
    pub fn execute_import(
        ...
        // Resolve module path
        let module_path = self.module_resolver.resolve_module(
            module_name,
            self.current_file.as_deref()
        )?;
        
        // Check for circular imports
        self.module_resolver.check_circular_import(&module_path, &self.import_stack)?;
        ...
        let module_scope = self.scope_manager.scopes().last()
        ...
        if modules.len() == 1 && modules[0] != "*" {
            ...
            if let Some(value) = module_namespace.get(item_name) {
                ...
                log_debug(&format!("Imported '{}' from module '{}'", item_name, module_name));
            } else {
            ...
            log_debug(&format!("Imported all from module '{}' as '{}'", module_name, import_name));
        } else {
