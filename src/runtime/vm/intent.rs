...
impl VirtualMachine {
    ...
    pub fn check_intent(&self, function_name: &str, action: &str, resource: &str) -> Result<(), RuntimeError> {
        self.intent_checker.check_action(function_name, action, resource)
    ...
    pub(super) fn map_stdlib_to_action(&self, function_name: &str, args: &[Value]) -> Option<(String, String)> {
        // Map I/O functions
        if function_name == "read_file" || function_name == "file_exists" || function_name == "is_file" || 
           function_name == "is_dir" || function_name == "list_dir" {
            if let Some(Value::String(path)) = args.first() {
                return Some(("fs.read".to_string(), path.clone()));
            }
        }
        if function_name == "write_file" || function_name == "append_file" || function_name == "delete" ||
           function_name == "mkdir" || function_name == "rmdir" {
            if let Some(Value::String(path)) = args.first() {
        ...
        if function_name == "http_get" || function_name == "http_post" || function_name == "tcp_connect" {
