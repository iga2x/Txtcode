...
use crate::runtime::permissions::{PermissionManager, PermissionResource, Permission};
...
impl VirtualMachine {
    ...
    pub fn check_permission(&self, resource: &PermissionResource, scope: Option<&str>) -> Result<(), RuntimeError> {
        self.permission_manager.check(resource, scope)
    ...
    pub fn check_permission_with_audit(&mut self, resource: &PermissionResource, scope: Option<&str>) -> Result<(), RuntimeError> {
        ...
        if let Some(current_frame) = self.call_stack.current_frame() {
            ...
            let action = match resource {
                ...
                PermissionResource::Process(_) => format!("process.exec"),
            };
        ...
        if let Some(token_id) = &self.active_capability {
            ...
            match self.capability_manager.check(token_id, resource, &action, scope) {
                Ok(()) => {
                    ...
                    // Capability check passed, skip permission check
                    return Ok(());
                }
                Err(cap_err) => {
        ...
        let action_str = format!("permission.check.{}", resource.to_string());
