        ...
        match stmt {
            ...
            Statement::IndexAssignment { target, index, value, .. } => {
                ...
                let obj_name = match target {
                    ...
                    _ => return Err(RuntimeError::new("Index assignment target must be an identifier".to_string())),
                };
                ...
                let updated = match (obj, idx) {
                    ...
                    (Value::Array(mut arr), Value::Integer(i)) => {
                        ...
                        if i < arr.len() {
                            ...
                            return Err(RuntimeError::new(format!("Array index {} out of bounds (len={})", i, arr.len())));
                        }
                        Value::Array(arr)
                    }
                    ...
                    (obj, idx) => return Err(RuntimeError::new(format!(
                        "Cannot index-assign: {:?}[{:?}]", obj, idx
                    ))),
                };
            ...
            Statement::CompoundAssignment { name, op, value, .. } => {
                let current = vm.get_variable(name)
            ...
            Statement::Break { .. } => Ok(Value::Null),
            Statement::Continue { .. } => Ok(Value::Null),
            Statement::FunctionDef { name, type_params: _, params, body, intent, ai_hint, allowed_actions, forbidden_actions, .. } => {
                // Convert CapabilityExpr to String for intent registration
                let allowed_strings: Vec<String> = allowed_actions.iter()
                    .map(|cap| cap.to_string())
                    .collect();
                let forbidden_strings: Vec<String> = forbidden_actions.iter()
                    .map(|cap| cap.to_string())
                    .collect();
                ...
                vm.set_global(name.clone(), Value::Function(
                    name.clone(),
                    params.clone(),
                    body.clone(),
                ...
                if let Some(intent_str) = intent {
                    let mut declaration = crate::runtime::intent::IntentDeclaration::new(intent_str.clone());
            ...
            Statement::Enum { name, variants, .. } => {
                ...
                log_debug(&format!("Registering enum '{}' with {} variants", name, variants.len()));
                vm.register_enum(name.clone(), variants.clone());
                Ok(Value::Null)
            }
            Statement::Struct { name, fields, .. } => {
                // Register struct definition
                log_debug(&format!("Registering struct '{}' with {} fields", name, fields.len()));
            ...
            Statement::Permission { resource, action, scope, .. } => {
