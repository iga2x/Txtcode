...
pub fn evaluate_member<VM: ExpressionVM>(
    ...
    if let Expression::Identifier(enum_type_name) = target.as_ref() {
        if let Some(variants) = vm.enum_defs().get(enum_type_name) {
            for (variant_name, _variant_value) in variants {
                if variant_name == name {
                    crate::tools::logger::log_debug(&format!("Accessing enum variant {}.{}", enum_type_name, name));
                    return Ok(Value::Enum(enum_type_name.clone(), name.to_string()));
                }
            }
            return Err(RuntimeError::new(format!("Enum '{}' has no variant '{}'", enum_type_name, name)));
        }
    }
    ...
    match obj {
        ...
        Value::String(s) => match name {
            ...
            _ => Err(RuntimeError::new(format!("String has no property '{}'. Use {}.{}() as a method call.", name, "str", name))),
        },
        Value::Array(arr) => match name {
            "length" | "len" => Ok(Value::Integer(arr.len() as i64)),
            "isEmpty" => Ok(Value::Boolean(arr.is_empty())),
            _ => Err(RuntimeError::new(format!("Array has no property '{}'. Use arr.{}() as a method call.", name, name))),
        },
        Value::Set(set) => match name {
            "length" | "len" | "size" => Ok(Value::Integer(set.len() as i64)),
            "isEmpty" => Ok(Value::Boolean(set.is_empty())),
            _ => Err(RuntimeError::new(format!("Set has no property '{}'.", name))),
        },
        Value::Enum(enum_name, _variant) => {
            Err(RuntimeError::new(format!("Cannot access members of enum value {}.{}", enum_name, _variant)))
        }
        _ => {
            Err(RuntimeError::new(format!("Member access only works on maps, structs, or enums, got {:?}", obj)))
        }
    }
}
