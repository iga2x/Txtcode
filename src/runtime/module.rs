...
impl ModuleResolver {
        ...
        if module_name.starts_with("./") || module_name.starts_with("../") {
            if let Some(current) = current_file {
                let current_dir = current.parent().ok_or_else(|| {
                    RuntimeError::new(
                        "Cannot resolve relative import: current file has no parent directory"
                            .to_string(),
                    )
                })?;
                ...
                return Err(RuntimeError::new(
                    "Relative imports require a current file path".to_string(),
                ));
            }
        }
    ...
    pub fn load_module(
        &mut self,
        module_path: &Path,
    ) -> Result<crate::parser::ast::Program, RuntimeError> {
        ...
        let source = fs::read_to_string(module_path).map_err(|e| {
            RuntimeError::new(format!("Failed to read module '{}': {}", path_str, e))
        })?;
        ...
        let tokens = lexer.tokenize().map_err(|e| {
            RuntimeError::new(format!("Failed to tokenize module '{}': {}", path_str, e))
        })?;

        let mut parser = Parser::new(tokens);
        let mut program = parser.parse().map_err(|e| {
            RuntimeError::new(format!("Failed to parse module '{}': {}", path_str, e))
        })?;
        ...
        if let Some(source_version) = metadata.get_version() {
            ...
            match compatibility
                .current_version()
                .is_compatible_with(source_version)
            {
                ...
                crate::runtime::compatibility::CompatibilityResult::Incompatible { reason } => {
                    return Err(RuntimeError::new(format!(
                        ...
                        path_str, source_version, reason
                    )));
                }
    ...
    pub fn check_circular_import(
        ...
        if import_stack.contains(&path_buf) {
            let cycle: Vec<String> = import_stack
                .iter()
