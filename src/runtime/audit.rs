...
impl std::fmt::Display for AuditResult {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            AuditResult::Error(msg) => write!(f, "error: {}", msg),
        }
    }
}
...
impl AuditEntry {
    ...
    pub fn with_ai_metadata(mut self, ai_meta: &AIMetadata) -> Self {
        ...
        if let Some(session) = &ai_meta.session {
            self.context
                .insert("ai_session".to_string(), session.clone());
        }
        if let Some(policy) = &ai_meta.policy_version {
            self.context
                .insert("ai_policy_version".to_string(), policy.clone());
        }
    ...
    pub fn to_json(&self) -> String {
        ...
        .unwrap();
        writeln!(json, "  \"timestamp_nanos\": {},", self.timestamp_nanos).unwrap();
        writeln!(json, "  \"action\": \"{}\",", escape_json(&self.action)).unwrap();
        writeln!(json, "  \"resource\": \"{}\",", escape_json(&self.resource)).unwrap();
        if let Some(perm) = &self.permission {
            writeln!(json, "  \"permission\": \"{}\",", escape_json(perm)).unwrap();
        } else {
            writeln!(json, "  \"permission\": null,").unwrap();
        }
        writeln!(json, "  \"result\": \"{}\",", self.result).unwrap();

        // Write context as JSON object
        writeln!(json, "  \"context\": {{").unwrap();
    ...
    pub fn from_permission_check(
        ...
        let action = format!("permission.check.{}", resource);
...
impl AuditTrail {
    ...
    pub fn export_json(&self) -> String {
        ...
        for (i, entry) in self.entries.iter().enumerate() {
            ...
            let indented: String = entry_json
                .lines()
                .map(|line| format!("  {}\n", line))
                .collect();
...
mod tests {
    use super::*;
    use crate::runtime::permissions::{PermissionError, PermissionResource};
    ...
    fn test_permission_check_logging() {
        ...
        // Test allowed
        trail
            .log_permission_check(&resource, Some("/tmp/test.txt"), Ok(()), None)
            .unwrap();
