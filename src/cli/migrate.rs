...
pub fn migrate_command(
    ...
    if let Some(dir) = directory {
        ...
        let reports =
            framework.migrate_directory(&dir_path, source_version, target_version, Some("*.tc"))?;
        ...
        if has_errors {
            return Err(RuntimeError::new(
                "Migration completed with errors. Review the report above.".to_string(),
            ));
        }
        ...
        if has_errors && strict {
            return Err(RuntimeError::new(
                "Migration completed with errors. Use --no-strict to allow warnings.".to_string(),
            ));
        }
    } else {
        return Err(RuntimeError::new(
            "No files or directory specified. Use --files or --directory.".to_string(),
        ));
    }
