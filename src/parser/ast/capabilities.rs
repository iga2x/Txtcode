...
impl CapabilityExpr {
    ...
    pub fn simple(resource: String, action: String, span: Span) -> Self {
        Self::Simple { resource, action, span }
    }
    ...
    pub fn to_string(&self) -> String {
        match self {
            CapabilityExpr::Simple { resource, action, .. } => {
                format!("{}.{}", resource, action)
            }
            CapabilityExpr::ResourceWildcard { action, .. } => {
                format!("*.{}", action)
            }
            CapabilityExpr::ActionWildcard { resource, .. } => {
                format!("{}.*", resource)
            }
            CapabilityExpr::FullWildcard { .. } => {
                "*.*".to_string()
            }
            CapabilityExpr::Scoped { resource, action, scope, .. } => {
                format!("{}.{}:{}", resource, action, scope)
            }
            CapabilityExpr::Tool { tool_name, .. } => {
                format!("tool:{}", tool_name)
            }
            CapabilityExpr::AllTools { .. } => {
                "tool:*".to_string()
            }
        }
    }
    ...
    pub fn matches(&self, resource: &str, action: &str) -> bool {
        match self {
            CapabilityExpr::Simple { resource: r, action: a, .. } => {
                r == resource && a == action
            }
            CapabilityExpr::ResourceWildcard { action: a, .. } => {
                a == action
            }
            CapabilityExpr::ActionWildcard { resource: r, .. } => {
                r == resource
            }
            CapabilityExpr::FullWildcard { .. } => {
                true
            }
            CapabilityExpr::Scoped { resource: r, action: a, scope: _scope, .. } => {
    ...
    pub fn matches_tool(&self, tool_name: &str) -> bool {
        match self {
            CapabilityExpr::Tool { tool_name: t, .. } => {
                t == tool_name
            }
            CapabilityExpr::AllTools { .. } => {
                true
    ...
    pub fn from_string(s: &str) -> Result<Self, String> {
        ...
        let (num_str, unit) = if s.len() > 1 {
            ...
            if last_char.is_alphabetic() {
                ...
                return Err(format!("Invalid duration format: {}. Expected number followed by unit (s, m, h, d)", s));
            }
        } else {
            return Err(format!("Invalid duration format: {}. Expected number followed by unit", s));
        };
        ...
        match unit.to_lowercase().next() {
            ...
            _ => Err(format!("Invalid duration unit: {}. Expected s, m, h, or d", unit)),
        }
    }
