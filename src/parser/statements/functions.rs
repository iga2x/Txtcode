...
pub fn parse_define(parser: &mut Parser) -> Result<Option<Statement>, String> {
        ...
        loop {
            ...
            if parser.check(crate::lexer::token::TokenKind::LeftBrace) {
                ...
                for field in &fields {
                    ...
                    let index_expr = crate::parser::ast::Expression::Index {
                        target: Box::new(crate::parser::ast::Expression::Identifier(
                            dest_name.clone(),
                        )),
            ...
            if parser.check(crate::lexer::token::TokenKind::Spread) {
                ...
                if parser.position + 2 < parser.tokens.len()
                    ...
                    && parser.tokens[parser.position + 1].kind
                        == crate::lexer::token::TokenKind::Dot
                    && parser.tokens[parser.position + 2].kind
                        == crate::lexer::token::TokenKind::Dot
                {
            ...
            if parser.check(crate::lexer::token::TokenKind::Assignment) {
                ...
                parser.advance();
                default_value = Some(crate::parser::expressions::operators::parse_expression(
                    parser,
                )?);
            }
    ...
    let has_intent = parser.check_keyword("doc")
        || parser.check_keyword("intent")
        || parser.check_keyword("hint")
        || parser.check_keyword("ai_hint")
        || parser.check_keyword("allowed")
        || parser.check_keyword("forbidden");
    ...
    while !parser.check_keyword("end") && !parser.is_at_end() {
        ...
        else if parser.check_keyword("hint")
            || parser.check_keyword("ai_hint")
            || parser.check_keyword("ai-hint")
            || parser.check_keyword("aihint")
        {
            parser.advance();
            parser.expect_arrow()?;
            if parser.check(crate::lexer::token::TokenKind::String) {
            ...
            while !parser.check(crate::lexer::token::TokenKind::RightBracket) {
                ...
                if parser.check(crate::lexer::token::TokenKind::String) {
                    ...
                    match CapabilityExpr::from_string(&cap_str, span) {
                        ...
                        Err(e) => {
                            return parser
                                .error(&format!("Invalid capability format in allowed: {}", e));
                        }
                    }
                } else {
            ...
            while !parser.check(crate::lexer::token::TokenKind::RightBracket) {
                ...
                if parser.check(crate::lexer::token::TokenKind::String) {
                    ...
                    match CapabilityExpr::from_string(&cap_str, span) {
                        ...
                        Err(e) => {
                            return parser
                                .error(&format!("Invalid capability format in forbidden: {}", e));
                        }
                    }
                } else {
